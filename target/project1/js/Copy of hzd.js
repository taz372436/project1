	            		...
	            		for(var t =2;t<=tds.length;t++){
	            			...
	            			if(columnid != "" && columnid != null){
	            				...
	            				row+="guestName:'"+$(tds[t]).html()+"',";
	    	       				...
	    	       				var dataObjectId = $(tds[3]).find("select").val();
                ...
                success: function(data) {
                    ...
                    $.each(data, function(index, html) {
    					for(var i=0;i<html.length;i++){
							$('#buhege').append(
								$('<option></option>').text(html[i].name).val(html[i].name)
							);
						}
                    });
//                    $("#buhege").multiselect('rebuild');
...
//					}
        ...
        function generateHtml(data){
        	...
        	for(var i=0;i<data.length;i++){
	        	...
	        	sHtml += '<div class="dd-handle dd-nodrag" style="height:45px;line-height:30px;"><b>回执单号:</b>'+data[i].single_receipt_num+'&nbsp;&nbsp;&nbsp;<b>生成日期:</b>'+formatterdate(data[i].single_time)+'&nbsp;&nbsp;&nbsp;<b>制单人:</b>'+data[i].single_player+'<a style="float:right;" class="btn btn-default light btn-sm DTTT_button_copy" onclick="javascript:dy('+data[i].id+')"/">打印</a><a style="float:right;" class="btn btn-default light btn-sm DTTT_button_copy" onclick="javascript:jjbl(\'hzdtable'+tidindex+'\')"/">保存</a></div>';
